...
PDF_MCP_SYSTEM_PROMPT = (
    ...
    "ВАЖНО: Возвращай ТОЛЬКО валидный JSON без дополнительного текста, комментариев или объяснений. Не используй Markdown, не добавляй комментарии в JSON. Возвращай только один JSON-объект без обертки в markdown code blocks.\n\n"
    "КРИТИЧНО: Все поля должны быть объектами или массивами согласно схеме. Поле 'volume' должно быть объектом с полями word_count, char_count, page_count, byte_size, reading_time_min, method. Поле 'complexity' должно быть объектом с полями score, level, estimated_grade, drivers, notes. НЕ используй простые строки для этих полей.\n\n"
    "Соблюдай схему и правила вывода из ТЗ.\n\n"
