...
async def test_category_consistency(pdf_path: str):
    ...
    normalized = [
        re.sub(r"\s+", " ", re.sub(r"[()/]", " ", cat.lower())).strip()
    ...
    if len(unique) == 1:
        ...
        print(
            f"  ✓ Категории семантически консистентны (различаются только форматированием)"
        )
        print(f"  ✓ Средний score: {avg_score:.2f}, разброс: {score_variance:.2f}")
        return True
...
async def main():
    if len(sys.argv) < 2:
        print("Использование: python -m project.tests.test_category <путь_к_pdf>")
        print("\nПримеры:")
        print(
            "  python -m project.tests.test_category project/tests/pdf_for_eval/llm-as-judge.pdf"
        )
        print(
            "  python -m project.tests.test_category project/tests/pdf_for_eval/Traffic\\ Signs.pdf"
        )
        sys.exit(1)
    ...
    print(f"{'='*60}")
    print(
        f"Тест 1 (Базовая категоризация): {'✓ PASS' if results[0] else '✗ FAIL'}"
    )
    print(
        f"Тест 2 (С существующими категориями): {'✓ PASS' if results[1] else '✗ FAIL'}"
    )
